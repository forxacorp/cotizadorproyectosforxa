...
async function boot() {
  const user = await CotizadorAuth.requireAdmin();
  if (!user) return;
  document.getElementById('header-actions').innerHTML = `
    <span class="user-pill">${user.email} · admin</span>
...
async function loadProjects() {
  ...
  list.innerHTML = `<table class="admin-table"><thead><tr>
    <th>Portada</th><th>Proyecto</th><th>Tipo financiamiento</th><th>Activo</th><th></th>
    ...
    return `<tr>
      ...
      <td style="white-space:nowrap;">
        ...
        <button class="icon-btn" title="${p.activo ? 'Desactivar' : 'Activar'}" data-toggle="${p.id}">${p.activo ? '⏻' : '▶'}</button>
      </td>
    </tr>`;
...
async function loadUnits() {
  ...
  const rows = await Promise.all(data.map(async u => {
    ...
    return `<tr data-id="${u.id}" data-table="${table}">
      ...
      <td style="white-space:nowrap;">
        <button class="icon-btn" data-save>&#10003;</button>
        <button class="icon-btn danger" data-del>&times;</button>
      </td>
    </tr>`;
  }));
...
async function saveUnitRow(tr) {
  ...
  if (error) { alert('Error guardando unidad: ' + error.message); return; }
  await loadUnits();
}

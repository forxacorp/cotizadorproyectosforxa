...
// esté en la tabla cotizador_admins (ver auth.js + schema.sql).
  ...
  if (!isAdmin) {
    document.querySelector('main.wrap').innerHTML = `
      <div class="card" style="text-align:center;padding:48px 32px;">
        <h3 class="centered">Tu cuenta no tiene permisos de administrador</h3>
        <p class="hint" style="max-width:440px;margin:0 auto 18px;">
          Tu usuario (<strong>${user.email}</strong>) puede usar el cotizador normalmente, pero para
          entrar aquí un administrador debe agregarte a <code>cotizador_admins</code>.
          Si eres tú quien administra, corre esto en el SQL Editor de Supabase:
        </p>
        <pre style="text-align:left;background:var(--crema);border:1px solid var(--border);border-radius:10px;padding:16px 18px;font-size:12.5px;overflow-x:auto;max-width:560px;margin:0 auto 20px;">insert into public.cotizador_admins (user_id, nombre)
select id, '${user.email}' from auth.users where email = '${user.email}'
